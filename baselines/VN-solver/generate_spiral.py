...
def spiral_layout(num_nodes, resolution=0.35, height_pixels=224,width_pixels=224, scale=100, center=(112, 112), dim=2, equidistant=False):
    pos = []
    if equidistant:
        chord = 1
        step = 0.5
...
def my_spiral_layout(num_nodes, resolution=0.35, height_pixels=224,width_pixels=224, scale=100, center=(112, 112), dim=2):
    dist = torch.arange(num_nodes, dtype=torch.float) #, device=this_device)
    angle = resolution * dist
    cos_angle = torch.cos(angle)
    sin_angle = torch.sin(angle)
    a = dist * torch.stack([cos_angle, sin_angle])
    pos = torch.transpose(dist * torch.stack([cos_angle, sin_angle]), 0, 1)

    # Find max length over all dimensions
    lim = 0  # max coordinate for all axes
    mean_per_row = torch.mean(pos, dim=0, keepdim=True)
    pos -= mean_per_row
    abs_pos = torch.abs(pos)
    max_abs_value = torch.max(abs_pos)
    pos *= 100 / max_abs_value
    pos += 112

    int_pos_x = []
    int_pos_y = []
    for each in pos:
        int_pos_x.append(int(each[0].round()))
        int_pos_y.append(int(each[1].round()))
        ...
        for matrix_text in matrices_text:
            if '1' in matrix_text:
                ...
                if count >= min_node and count < max_node:
                    ...
                    if 'spiral' in visualization:
                        x, y = my_spiral_layout(len(matrix)) #, resolution=0.35, height_pixels=224, width_pixels=224, scale=100, center=(112, 112), dim=2)
